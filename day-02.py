def checksum(rows):
    ...
    for row in rows:
        ...
        differences.append(sorted_row[-1] - sorted_row[0])
    ...
    for row in rows:
        ...
        for item in row:
            if pair:
                break
            for other in row:
                if item != other and (item / other) % 1 == 0.0:
                    pair = (item, other)
                    break
...
if __name__ == "__main__":
    ...
    sheet = """1136	1129	184	452	788	1215	355	1109	224	1358	1278	176	1302	186	128	1148
    ...
    sheet = [[int(item) for item in row.split()] for row in sheet.split("\n")]
